...
! Usage example:
! 1. add it to gui function in GUI.f90, such as drawmolgui, ...
    ...
    subroutine mouse_rotate(id)
        ...
        do while (.true.)
            if (PeekMessage(current_msg, active_window, 0, 0, PM_REMOVE) == TRUE) then
                ...
                case (WM_LBUTTONDOWN)
                    startX = LOWORD(current_msg%lParam)
                    startY = HIWORD(current_msg%lParam)
                    isDragging = .true.
                
                case (WM_MOUSEMOVE)
                    if (isDragging) then
                        currentX = LOWORD(current_msg%lParam)
                        currentY = HIWORD(current_msg%lParam)
